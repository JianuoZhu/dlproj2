...
module Race_Mode(
    ...
    always @(posedge clk, negedge rst_n) begin
        if(!rst_n)begin
        ...
        else if(former_user_select != user_select)begin
            former_user_select <= user_select;
            user_display <= 8'b0000_0000;
            finished <= 1'b0;
            segments <= 8'b0000_0000;
            ...
            case (current_state)
                WAITING: begin
                    if (debounced_switch_song && (!switch_flag)) begin
                        index <= index0;
                        case(music)
                            song1: music <= song2;
                            song2: music <= song3;
                ...
                FINISHED: begin
                    if(wrong_counter < ((WAIT_MAX * max_index[music]) >> 2))begin
                        segments <= A_segment;
                    end
                    else if(wrong_counter < ((WAIT_MAX * max_index[music]) >> 1))begin
                        segments <= B_segment;
                    end
                    else if(wrong_counter < ((WAIT_MAX * max_index[music]) >>1 + ((WAIT_MAX * max_index[music]) >> 2)))begin
                        segments <= C_segment;
                    end
                    else begin
                        segments <= D_segment;
